...
module.exports = class {
    ...
    this.columns.forEach((col, x) => {
      ...
      for (let y = colBot; y > colTop; y--) {
        ...
        colors[index] = 
          ...
          || percent < 0.5
            ? parseColor(this.getColor(), { brightness: col.brightness })
            : percent < 0.75
              ? parseColor(this.getColor(), { brightness: 0.25 })
              : parseColor(this.getColor(), { brightness: 0.1 })
        i++
      }
    })
    this.colors = colors
  ...
  getColor() {
    return Math.random() < 0.5 ? 'green' : 'slime'
  }
